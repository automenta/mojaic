...
public class MosaicPane extends Container implements MosaicPaneListener {
    ...
    private final MosaicEngine<Component> layoutEngine;
    private final Surface<Component> surface;

    private final FadeDecorator ghoster = new FadeDecorator(0.5f);
    private final DropShadowDecorator shadower = new DropShadowDecorator(10, 10, 10);

    private final MosaicPaneListenerList mosaicPaneListeners = new MosaicPaneListenerList();
    ...
    public <T> MosaicPane(MosaicEngine<Component> engine, Surface<Component> surface) {
        ...
        if (engine == null || surface == null) {
            ...
            MosaicSurfaceBuilder<Component> builder = new MosaicSurfaceBuilder<>();
        ...
        SurfaceListener<Component> listener = (changeType, c, id, r1, r2) -> {
            switch (changeType) {
                case REMOVE_DISCARD: {
                    remove(c);
                    break;
