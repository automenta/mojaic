...
class MosaicEngineImpl<T> implements MosaicEngine<T> {
     ...
     * List of Surfaces
     */
    private final List<SurfacePriviledged<T>> surfaces = new ArrayList<>();

    /**
     * Map of registered Surfaces to their respective visitation tools
     */
    private final Map<SurfacePriviledged<T>, PathVisitor<T>[]> visitors = new HashMap<>();
    ...
    public void addSurface(Surface<T> surface) {
        ...
        visitors.put(s, new PathVisitor[]{
                new LayoutVisitor<>(s),
                new AlignmentVisitor<>(s),
                new WeightAdjustmentVisitor<>(s)
        });
    ...
    private List<Divider<T>> getAllVerticalDividersInRange(List<Divider<T>> dividers, double x1, double x2, Divider<T> exclude) {
        List<Divider<T>> searchResults = new ArrayList<>();
    ...
    List<Divider<T>> getAllOverlappingDividers(SurfacePriviledged<T> surface, Divider<T> divider) {
        List<Divider<T>> l = new ArrayList<>();
        ...
        for (Node<T> n : surface.getNodeList()) {
            ...
            if (type != ConnectType.BOTH) {
                ...
                if (ny + nh < surfaceArea.getHeight() - boundaryY &&
                    ...
                    div = new Divider<>(nx, ny + nh, nw, false);
                    div.dividerSize = surface.getDividerSize();
                    div.addPrevious(n);
            ...
            if (type != ConnectType.BOTH) {
                ...
                if (nx + nw < surfaceArea.getWidth() - boundaryX &&
                    ...
                    div = new Divider<>(nx + nw, ny, nh, true);
                    div.dividerSize = surface.getDividerSize();
                    div.addPrevious(n);
                    n.nextVertical = div;
                    verticalDividers.add(div);
    ...
    private void connectDividersForAdd(SurfacePriviledged<T> surface, Node<T> source, Node<T> target, Position p) {
        ...
        switch (p) {
            case NORTH: {
                ...
                Divider<T> newDiv = new Divider<>(source.r.x, source.r.getMaxY(), source.r.width, false);
            ...
            case SOUTH: {
                ...
                Divider<T> newDiv = new Divider<>(source.r.x, target.r.getMaxY(), source.r.width, false);
                newDiv.dividerSize = surface.getDividerSize();
                newDiv.r.height = newDiv.dividerSize;
            ...
            case EAST: {
                ...
                Divider<T> newDiv = new Divider<>(target.r.getMaxX(), target.r.y, source.r.height, true);
                newDiv.dividerSize = surface.getDividerSize();
            ...
            case WEST: {
                ...
                Divider<T> newDiv = new Divider<>(source.r.getMaxX(), target.r.y, target.r.height, true);
                newDiv.dividerSize = surface.getDividerSize();
                newDiv.r.width = newDiv.dividerSize;
                newDiv.addPrevious(source);
...
//                dragYOffset = e.type == ElementType.DIVIDER && ((Divider<T>) e).isVertical && alt != null ? y - alt.r.y : y - e.r.y;
        ...
        private Position getDragQuadrant(Rectangle2D.Double r, double x, double y) {
            ...
            double rwDiv4 = rw / 4;
            double rhDiv3 = rh / 3;
            if (x < rwDiv4 && y > rhDiv3 && y < rh - rhDiv3) {
                return Position.WEST;
            } else if (x > rw - rwDiv4 && y > rhDiv3 && y < rh - rhDiv3) {
                return Position.EAST;
            } else if (y < rwDiv4 && x > rhDiv3 && x < rw - rhDiv3) {
                return Position.NORTH;
            } else if (y > rh - rwDiv4 && x > rhDiv3 && x < rw - rhDiv3) {
    ...
    public static boolean distanceWithin(double x1, double y1, double x2, double y2, double maxDist) {
        x1 -= x2;
        y1 -= y2;
        return (x1 * x1 + y1 * y1) <= maxDist * maxDist;
    }
}
